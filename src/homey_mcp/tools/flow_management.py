...
class FlowManagementTools:
    ...
    async def handle_get_flows(self, arguments: Dict[str, Any]) -> List[TextContent]:
        ...
        try:
            ...
            return [TextContent(
                type="text",
                text=f"❌ Fout bij ophalen flows: {str(e)}"
            )]
    ...
    async def handle_trigger_flow(self, arguments: Dict[str, Any]) -> List[TextContent]:
        ...
        try:
            ...
            if flow_id not in flows:
                return [TextContent(
                    type="text",
                    text=f"❌ Flow met ID '{flow_id}' niet gevonden"
                )]
            ...
            if success:
                return [TextContent(
                    type="text",
                    text=f"✅ Flow '{flow_name}' succesvol gestart"
                )]
            else:
                return [TextContent(
                    type="text",
                    text=f"❌ Kon flow '{flow_name}' niet starten"
                )]
                
        except Exception as e:
            return [TextContent(
                type="text",
                text=f"❌ Fout bij starten flow: {str(e)}"
            )]
    ...
    async def handle_find_flow_by_name(self, arguments: Dict[str, Any]) -> List[TextContent]:
        ...
        try:
            ...
            if matching_flows:
                ...
                return [TextContent(
                    type="text",
                    text=f"Geen flows gevonden met naam '{arguments['flow_name']}'"
                )]
                
        except Exception as e:
            return [TextContent(
                type="text",
                text=f"❌ Fout bij zoeken flows: {str(e)}"
            )]
