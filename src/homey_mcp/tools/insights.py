...
class InsightsTools:
    ...
    def get_tools(self) -> List[Tool]:
        """Geef alle insights tools terug."""
        return [
            Tool(
                name="get_device_insights",
                description="Haal historical data op voor device capability over een periode",
                inputSchema={
                    "type": "object",
                    "properties": {
                        "device_id": {
                            "type": "string",
                            "description": "Het ID van het device"
                        },
                        "capability": {
                            "type": "string", 
                            "description": "Capability naam (bv. measure_temperature, dim, onoff, measure_power)"
                        },
                        "period": {
                            "type": "string",
                            "enum": ["1h", "6h", "1d", "7d", "30d", "1y"],
                            "default": "7d",
                            "description": "Tijdsperiode voor data"
                        },
                        "resolution": {
                            "type": "string",
                            "enum": ["1m", "5m", "1h", "1d"],
                            "default": "1h", 
                            "description": "Data resolutie"
                        }
                    },
            ...
            Tool(
                ...
                description="Haal energie verbruik data op van devices",
                inputSchema={
                    "type": "object",
                    "properties": {
                        "period": {
            ...
            Tool(
                ...
                description="Real-time dashboard data voor monitoring",
                inputSchema={
                    "type": "object",
                    "properties": {
                        "metrics": {
    ...
    async def handle_get_device_insights(self, arguments: Dict[str, Any]) -> List[TextContent]:
        """Handler voor get_device_insights tool."""
        try:
            ...
            response_text = f"📊 Device Insights voor {device_id} - {capability}\n\n"
            response_text += f"📅 Periode: {period} | Resolutie: {resolution}\n\n"
            
            if capability == "measure_temperature":
                avg_temp = round(random.uniform(19, 23), 1)
                response_text += f"🌡️ **Temperatuur Data:**\n"
                response_text += f"• Gemiddelde: {avg_temp}°C\n"
                response_text += f"• Min: {avg_temp - 2:.1f}°C\n"
                response_text += f"• Max: {avg_temp + 2:.1f}°C\n"
            elif capability == "dim":
                avg_brightness = round(random.uniform(0.3, 0.8), 2)
                response_text += f"💡 **Helderheid Data:**\n"
                response_text += f"• Gemiddelde helderheid: {avg_brightness*100:.0f}%\n"
                response_text += f"• Aan-tijd: {random.randint(4, 8)} uur/dag\n"
                response_text += f"• Piek gebruik: 19:00-22:00\n"
            elif capability == "measure_power":
                avg_power = round(random.uniform(20, 80), 1)
                response_text += f"⚡ **Power Data:**\n"
                response_text += f"• Gemiddeld verbruik: {avg_power}W\n"
                response_text += f"• Dagelijks verbruik: {avg_power * 24 / 1000:.1f} kWh\n"
                response_text += f"• Geschatte kosten: €{avg_power * 24 * 0.30 / 1000:.2f}/dag\n"
            else:
                response_text += f"📈 **General Data:**\n"
                response_text += f"• Data points: {random.randint(50, 200)}\n"
                response_text += f"• Laatste waarde: {random.uniform(0, 100):.1f}\n"
            
            response_text += f"\n💡 *Demo data - echte Homey Insights API nog te implementeren*"

            return [TextContent(type="text", text=response_text)]

        except Exception as e:
            return [TextContent(type="text", text=f"❌ Fout bij device insights: {str(e)}")]

    async def handle_get_energy_insights(self, arguments: Dict[str, Any]) -> List[TextContent]:
        """Handler voor get_energy_insights tool."""
        try:
            period = arguments.get("period", "7d")
            ...
            response_text = f"🔋 Energie Insights - {period}\n\n"
            
            # Demo energy data
            total_consumption = round(random.uniform(50, 200), 1)
            total_cost = round(total_consumption * 0.30, 2)
            
            response_text += f"📊 **Totaal Overzicht:**\n"
            response_text += f"• Totaal verbruik: {total_consumption} kWh\n"
            response_text += f"• Geschatte kosten: €{total_cost}\n"
            response_text += f"• Gemiddeld per dag: {total_consumption/7:.1f} kWh\n\n"
            
            if group_by == "device":
                response_text += f"📱 **Top Verbruikers:**\n"
                devices = ["Wasmachine", "Koelkast", "TV", "Verlichting", "Verwarmimg"]
            ...
            response_text += f"\n💡 *Demo data - connecteer met echte Homey energy monitoring*"
            
            return [TextContent(type="text", text=response_text)]

        except Exception as e:
            return [TextContent(type="text", text=f"❌ Fout bij energie insights: {str(e)}")]

    async def handle_get_live_insights(self, arguments: Dict[str, Any]) -> List[TextContent]:
        """Handler voor get_live_insights tool."""
        try:
            ...
            return [TextContent(type="text", text=f"❌ Fout bij live insights: {str(e)}")]
