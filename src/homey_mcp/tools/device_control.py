...
class DeviceControlTools:
    ...
    async def handle_get_devices(self, arguments: Dict[str, Any]) -> List[TextContent]:
        ...
        try:
            ...
            return [TextContent(
                type="text", 
                text=f"Fout bij ophalen devices: {str(e)}"
            )]
    ...
    async def handle_control_device(self, arguments: Dict[str, Any]) -> List[TextContent]:
        ...
        try:
            ...
            if success:
                ...
                return [TextContent(
                    type="text",
                    text=f"❌ Kon capability van '{device_name}' niet zetten"
                )]
                
        except Exception as e:
            return [TextContent(
                type="text",
                text=f"❌ Fout bij bedienen device: {str(e)}"
            )]
    ...
    async def handle_get_device_status(self, arguments: Dict[str, Any]) -> List[TextContent]:
        ...
        try:
            ...
            return [TextContent(
                type="text",
                text=f"❌ Fout bij ophalen device status: {str(e)}"
            )]
    ...
    async def handle_find_devices_by_zone(self, arguments: Dict[str, Any]) -> List[TextContent]:
        ...
        try:
            ...
            return [TextContent(
                type="text",
                text=f"❌ Fout bij zoeken devices: {str(e)}"
            )]
    ...
    async def handle_control_lights_in_zone(self, arguments: Dict[str, Any]) -> List[TextContent]:
        ...
        try:
                ...
                try:
                    ...
                    if action == "on":
                        ...
                        if brightness is not None and "dim" in device.get("capabilitiesObj", {}):
                            await self.homey_client.set_capability_value(device_id, "dim", brightness / 100)
                        
                        results.append(f"✅ {device_name}: aangezet" + 
            ...
            return [TextContent(
                type="text",
                text=f"❌ Fout bij bedienen lichten: {str(e)}"
            )]
