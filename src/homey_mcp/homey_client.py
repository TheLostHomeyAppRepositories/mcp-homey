...
class HomeyAPIClient:
    ...
    async def set_capability_value(self, device_id: str, capability: str, value: Any) -> bool:
        ...
        if self.config.offline_mode or self.config.demo_mode:
            logger.info(
                f"Demo mode: Device {device_id} capability {capability} zou worden gezet naar {value}"
            )
            return True

        try:
            # Correct endpoint format volgens Homey Web API documentatie
            endpoint = f"/api/manager/devices/device/{device_id}/capability/{capability}"
            payload = {"value": value}

            # Gebruik POST methode - PUT geeft 400 Bad Request
